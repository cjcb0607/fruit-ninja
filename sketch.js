...
function setup() {
  ...
  //crea la espada/cuchillo
   ...
   knife.addImage("cuchillo",knifeImage);
   knife.scale=0.7
   knife.addImage("gameoi",gameOverImage);
...
function draw() {
  ...
  if(gameState===PLAY){
    ...
    else
    {
      ...
      if(monsterGroup.isTouching(knife)){
        ...
        knife.changeImage("gameoi",gameOverImage);
        knife.scale=2;
        knife.x=300;
        knife.y=300;
      }
  ...
  if(gameState===END&&mousePressedOver(knife)) {
      reset();
    }
  drawSprites();
...
function reset(){
  gameState= PLAY;
  score= 0
  knife.changeImage("cuchillo",knifeImage);
  knife.scale=0.7
  }
